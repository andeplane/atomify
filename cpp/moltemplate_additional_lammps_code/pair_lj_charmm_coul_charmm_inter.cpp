...
void PairLJCharmmCoulCharmmInter::settings(vector<string>& vArgs)
{
  ...
  while (i < vArgs.size())
  {
    ...
    { 
      if (vArgs[i] == "both") {
        utils::bounds(FLERR,vArgs[i+1],1,MAXTAGINT-1,b_mol_lo,b_mol_hi,error);
        utils::bounds(FLERR,vArgs[i+2],1,MAXTAGINT-1,b_jmol_lo,b_jmol_hi,error);
        mol_lo = b_mol_lo;
        mol_hi = b_mol_hi;
        jmol_lo = b_jmol_lo;
        jmol_hi = b_jmol_hi;
  ...
  cut_lj_inner = utils::numeric(FLERR,vArgs[0].c_str(),false,lmp);
  cut_lj = utils::numeric(FLERR,vArgs[1].c_str(),false,lmp);
  cut_coul_and_lj_differ = 0;
  cut_coul_inner = cut_lj_inner;
  cut_coul = cut_lj;
  if (vArgs.size() == 4) {
    cut_coul_inner = utils::numeric(FLERR,vArgs[2].c_str(),false,lmp);
    cut_coul = utils::numeric(FLERR,vArgs[3].c_str(),false,lmp);
...
void PairLJCharmmCoulCharmmInter::coeff(int narg, char **arg)
{
  ...
  if (coeff_style == COEFF_AB) {
    A_one = utils::numeric(FLERR,arg[2],false,lmp);
    B_one = utils::numeric(FLERR,arg[3],false,lmp);
    A14_one = A_one;
    B14_one = B_one;
    #ifdef ENABLE_INTER_DIF
    ADIF_one = A_one;
    ...
    #endif
    if (narg >= 4+2) {
      A14_one = utils::numeric(FLERR,arg[4],false,lmp);
      B14_one = utils::numeric(FLERR,arg[5],false,lmp);
      if (narg == 6+2) {
        #ifdef ENABLE_INTER_DIF
        ADIF_one = utils::numeric(FLERR,arg[6],false,lmp);
        BDIF_one = utils::numeric(FLERR,arg[7],false,lmp);
        #else
           ...
           (coeff_style == COEFF_EPS_SIG_1n2))
  {
    if (narg >= 4+2) {
      ...
      if (narg == 6+2) {
        #ifdef ENABLE_INTER_DIF
        epsDIF_one = utils::numeric(FLERR,arg[6],false,lmp);
        sigmaDIF_one = utils::numeric(FLERR,arg[7],false,lmp);
        #else
        sprintf(err_msg, "This version of LAMMPS was compiled without support for\n"
                "custom inter-vs-intra-molecular pairwise forces.  To fix this,\n"
                "the \"ENABLE_INTER_DIF\" macro must be defined in source\n"
  ...
  { 
    ...
    kappa14_one = utils::numeric(FLERR,arg[4],false,lmp);
    lambda14_one = 1.0;

    #ifdef ENABLE_INTER_DIF
    kappaDIF_one = utils::numeric(FLERR,arg[4],false,lmp);
    lambdaDIF_one = 1.0;
    #endif
    ...
    if (narg >= 6+2) {
      ...
      kappa14_one = utils::numeric(FLERR,arg[7],false,lmp);
      if (narg == 9+2) {
        #ifdef ENABLE_INTER_DIF
        epsDIF_one = utils::numeric(FLERR,arg[8],false,lmp);
        sigmaDIF_one = utils::numeric(FLERR,arg[9],false,lmp);
        kappaDIF_one = utils::numeric(FLERR,arg[10],false,lmp);
        #else
        sprintf(err_msg, "This version of LAMMPS was compiled without support for\n"
                "custom inter-vs-intra-molecular pairwise forces.  To fix this,\n"
                "the \"ENABLE_INTER_DIF\" macro must be defined in source\n"
  ...
  {
    kappa_one = utils::numeric(FLERR,arg[4],false,lmp);
    lambda_one = utils::numeric(FLERR,arg[5],false,lmp);
    ...
    if (narg >= 8+2) {
      eps14_one = utils::numeric(FLERR,arg[6],false,lmp);
      sigma14_one = utils::numeric(FLERR,arg[7],false,lmp);
      kappa14_one = utils::numeric(FLERR,arg[8],false,lmp);
      lambda14_one = utils::numeric(FLERR,arg[9],false,lmp);
      if (narg == 12+2) {
        #ifdef ENABLE_INTER_DIF
        epsDIF_one = utils::numeric(FLERR,arg[10],false,lmp);
        sigmaDIF_one = utils::numeric(FLERR,arg[11],false,lmp);
        kappaDIF_one = utils::numeric(FLERR,arg[12],false,lmp);
        lambdaDIF_one = utils::numeric(FLERR,arg[13],false,lmp);
        #else
        sprintf(err_msg, "This version of LAMMPS was compiled without support for\n"
                "custom inter-vs-intra-molecular pairwise forces.  To fix this,\n"
                "the \"ENABLE_INTER_DIF\" macro must be defined in source\n"
