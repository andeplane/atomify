...
test("SSAO quality is applied to the real pass and survives saved settings", async ({
  ...
  await page.evaluate(async () => {
    ...
    const pass = v.renderer.postProcessingManager.n8aoPass;
  ...
  await canvas.screenshot({ path: "/tmp/atomify-ssao-old.png" });
  ...
  await expect
    .poll(passConfig)
    .toEqual({ samples: 16, denoise: 8, pixels: true });
  ...
  await page
    ...
    .click();
  await canvas.screenshot({ path: "/tmp/atomify-ssao-detailed.png" });
