...
class PyoliteRemoteKernel {
    ...
    async initKernel(options) {
        ...
        await this._pyodide.runPythonAsync(`
      await piplite.install(['matplotlib', 'pandas', 'ipykernel', 'atomify-lammps-logfile'], keep_going=True);
    ...
    `);
        ...
        if (options.mountDrive && this._localPath) {
            await this._pyodide.runPythonAsync(`
        await piplite.install(['atomify-lammps-logfile'], keep_going=True);
        import os;
...
//# sourceMappingURL=packages_pyolite-kernel_lib_worker_js.7dfc1a4.js.map