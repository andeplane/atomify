...
bool Fix::trySync(LAMMPS_NS::FixAveTime *fix) {
  ...
  if(m_nextValidTimestep+1 == m_lmp->update->ntimestep) { // m_nextValidTimestep+1 because fix_atomify is invoked before all other fixes, so we need the next timestep
    if(mode == SCALAR) {
      ...
      auto& values = fix->getvalues();
      for(int i=0; i<nvalues; i++) {
        auto type = getType(values[i].which, values[i].id);
        ...
        if(type==ComputeRDF) {
          LAMMPS_NS::ComputeRDF *compute_rdf = dynamic_cast<LAMMPS_NS::ComputeRDF *>(m_lmp->modify->get_compute_by_id(values[i].id));
